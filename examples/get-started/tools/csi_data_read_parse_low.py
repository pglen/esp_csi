...
def   onclick3():
      global train, recog
      logx.print("Train started with '", tlabel.text(), "'")
...
class FormWidget(QWidget):
    ...
    def __init__(self, parent):
        ...
        self.button3a = QPushButton("tcnt+")
...
class csi_data_graphical_window(QMainWindow):
    def __init__(self):
        ...
        self.plotWidget_ted.setYRange(-150, 150)
        #self.plotWidget_ted.addLegend((10,1))
        self.plotWidget_ted.setTitle('CSI Scatter Plot Low Pass')
    ...
    def update_data(self):
        ...
        global mxx2x, hitx, mxx2s, hits
        global ztime

        for aa in range(len(mxx2s)-1, 0, -1):
            bb = mxx2s[aa]
            if bb < mxx2x:
               mxx2x = bb
               hitx = hits[aa]
               ztime = 0
               zlabel.setText(" %s" % hitx)
               xlabel.setText("%.2f" % mxx2x)
               mxx2s = []
               hits = []
               break
...
def fill(csi_raw_datax, outdata, outdata2):
    ...
    try:
        for i in range(csi_vaid_subcarrier_len // 2):
            ...
            #val = math.sqrt(SQR(csi_raw_datax[i * 2])+SQR(csi_raw_datax[i * 2 +1]))
            #val = deangle(csi_raw_datax[i * 2], csi_raw_datax[i * 2 +1])

            outdata[i][0] += (val - outdata[i][0]) / dampen

            #val2 =   csi_raw_datax[i * 2+1]
            #outdata2[i][0] += (val2 - outdata2[i][0])
        ...
        if train:
            trainx(outdata, outdata2)

        if recog:
            recogx(outdata, outdata2)
    except:
        print("fill", sys.exc_info())

dcnt = 0

def trainx(csi_raw_data1, csi_raw_data2):

    global datacnt, train, offsx, traindict, traindict2

    #print("Train", datacnt)
    for aa in range(len(csi_raw_data1)):
        #traindict[tlabel.text()][datacnt][aa] = csi_raw_datax[aa*2]
        traindict[tlabel.text()][datacnt][aa] = csi_raw_data1[aa][0]
        #            math.sqrt(SQR(csi_raw_datax[aa*2]) + SQR(csi_raw_datax[aa*2-1]) )

        traindict2[tlabel.text()][datacnt][aa] = csi_raw_data2[aa][0]
    ...
    if datacnt >= CSI_TRAIN_SIZE:
        ...
        logx.print("Training added at %d" % offsx)
        offsx += datacnt +2
        datacnt = 0
        onclick3a()
...
def recogx(csi_datax, csi_datax2):

    global incnt
    for aa in range(len(csi_datax)):
        currsig[incnt % CSI_TRAIN_SIZE][aa] = csi_datax[aa][0]
    ...
    for aa in  traindict.keys():
        ...
        for bb in range(len(traindict[aa])):
            ...
            for cc in range(len(currsig[idx])):
                ...
                #ddd.append(SQR(currsig2[idx][cc] - traindict2[aa][bb][cc]))
                #print ('%.2f#%.2f=%.2f  ' \
    ...
    logx.print(strpad(hit, 8), strpad('%.2f' % mxx2, 8), end='  ')
    global dcnt
    dcnt += 1
    if dcnt % 4 == 0:
        logx.print()
