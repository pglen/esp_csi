...
class csi_data_graphical_window(QMainWindow):
    def __init__(self):
        ...
        '''
        global mimage, mlabel
        mimage = Image.new("RGBA", (800, 300), color=(000,000,000) )
        mlabel = QLabel(self)
        pix = QPixmap.fromImage(ImageQt.ImageQt(mimage))
        ...
        self.sub_layout2.addWidget(mlabel)
    ...
    def update_data(self):
        ...
        scale, mxx = getscale(csi_data_array[-1])
        scale = 2 ; mxx = 64
        #print(mxx, scale)
        ...
        ptr = list(bw.getdata())
        #print(ptr)
        #scale, mxx = getscale(csi_data_array2[-1])
        for bb in range(200):
            xx = csi_data_array2[-1][bb]
            # normalize
            iii = int((xx+mxx/2) * scale) & 255
            if not xx: iii = 0
            ptr[bb] = (0, iii, 0)
...
def getscale(csi_data_arrayx):
    scale = 3;  mxx = 0
    for bbb in range(200):
        xx = abs(csi_data_arrayx[bbb]) # + 128
        if xx > mxx:
            mxx = xx
    if mxx:
        scale = 255 / mxx
...
def drawmain():
    ...
    global mimage, mlabel
    ptr = list(mimage.getdata())
    for bb in range(0, 200, 1):
        scale, mxx = getscale(csi_data_array[bb])
        #logx(scale)
        for cc in range(0, 200, 1):
            #ptr[row + cc] = (random.randint(0,255), \
            #    random.randint(0,255), random.randint(0,255))
            xx = csi_data_array[bb][cc]
            iii = int((xx+mxx/2) * scale) & 255
            if not xx: iii = 0
            ptr[bb*800 + cc] = (0, iii, 0)

        scale, mxx = getscale(csi_data_array2[bb])
        #logx(scale)
        for cc in range(0, 200, 1):
            xx = csi_data_array2[bb][cc]
            iii = int((xx+mxx/2) * scale) & 255
            if not xx: iii = 0
            ptr[bb*800 + 250 + cc] = (0, iii, 0)
...
def drawmount():
    ...
    draw.rectangle((0, 0, 800, 300), fill=(0, 0, 0, 255))

    try:
        ptr = list(mimage.getdata())
        for cc in range(0, 200, 1):
            ptr[200*800 + cc * 4 ] = (200, 0,0)
            ptr[200*800 + cc * 4 +1] = (200, 0,0)
            ptr[200*800 + cc * 4 +2] = (200, 0,0)
            #ptr[200*800 + cc * 4 +3] = (200, 0,0)

        for bb in range(199, 150, -1):
            scale, mxx = getscale(csi_data_array[bb])
            ...
            for cc in range(0, 200, 1):
                ...
                if xx == 0:
                    continue
                #xx = int((xx+mxx/2) * scale) & 255
                iii = int(-200 - xx * 2 ) + int(bb*2)
                jjj = 100 + cc * 4 - iii // 1
                if iii >= 0 and iii < 300:
                    if jjj >= 0 and jjj < 800:
                        #ptr[iii*800 + jjj] = ccc
                        for ddd in range(2):
                            for eee in range(1):
                                ptr[(iii+eee)*800 + jjj+ddd] = ccc
...
def fill(csi_raw_datax, outdata, outdata2):
    ...
    try:
        for i in range(csi_vaid_subcarrier_len // 2):
            ...
            #outdata[-1][i] = csi_raw_datax[i * 2]
            #val =   csi_raw_datax[i * 2-1] - csi_raw_datax[i * 2]
            #val =   csi_raw_datax[i * 2]
            #val =   csi_raw_datax[i * 2]
            val =   csi_raw_datax[i * 2] + csi_raw_datax[i * 2-1]

            outdata[-1][i] += (val - outdata[-1][i]) / 2
            #outdata[i][0] = math.sqrt(SQR(csi_raw_datax[i * 2])+SQR(csi_raw_datax[i * 2 -1]))

            val =   csi_raw_datax[i * 2-1]
            outdata2[-1][i] += (val - outdata2[-1][i]) / 2
            #outdata2[i][0] = deangle(csi_raw_datax[i * 2], csi_raw_datax[i * 2 -1])

            currsum[-1][i] += (outdata[-1][i] - currsum[-1][i]) / 2
        ...
        for i in range(csi_vaid_subcarrier_len // 2):
            try:
                ...
                #zsum += csi_data_array[-1][i] - csi_data_array2[-1][i]
                zsum += currsum[-1][i]
            except:
                pass
        ...
        global gval
        idx = 44
        gval += (csi_data_array[-1][idx] - gval) / 10
        #outdata4[prog][0] = gval
        outdata4[prog][0] = zsum

        global train, datacnt, offsx
        if train:
            trainx(csi_raw_datax)
...
def trainx(csi_raw_datax):

    global datacnt, train, offsx

    #print("Train", datacnt)
    for aa in range(len(csi_raw_datax)// 2):
        #traindict[tlabel.text()][datacnt][aa] = csi_raw_datax[aa*2]
        traindict[tlabel.text()][datacnt][aa] = \
            math.sqrt(SQR(csi_raw_datax[aa*2]) + SQR(csi_raw_datax[aa*2-1]) )

        traindict2[tlabel.text()][datacnt][aa] = \
            deangle(csi_raw_datax[aa * 2], csi_raw_datax[aa * 2 -1]) * 4

        #csi_raw_datax[aa*2-1]
